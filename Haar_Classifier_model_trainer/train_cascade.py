...
def evaluate_model(model_path, test_pos_dir, test_neg_dir):
    ...
    # scaleFactor=1.3 inclus car pertinent pour le Raspberry Pi Zero V1 (plus rapide)
    scaleFactors = [1.05, 1.1, 1.2, 1.3]
    minNeighborsList = [3, 5, 7]
    ...
    total_configs = len(scaleFactors) * len(minNeighborsList)

    all_results = []
    best_f1 = -1
    best_idx = 0

    print(f"Recherche des meilleurs paramètres ({total_configs} configurations)...")
    config_idx = 0
    for sf in scaleFactors:
        for mn in minNeighborsList:
            config_idx += 1
            print(f"  [{config_idx}/{total_configs}] SF={sf}, MN={mn}...")
    ...
    # ── Tableau comparatif de toutes les configurations ──
    print(f"\n  Comparaison des {total_configs} configurations testées :")
    print(f"  {'SF':>6} {'MN':>4}  {'Recall':>8} {'Préc.':>8} {'F1':>7} {'Spéc.':>8} {'FP/img':>8} {'IoU':>6}")
    print(f"  {'─'*6} {'─'*4}  {'─'*8} {'─'*8} {'─'*7} {'─'*8} {'─'*8} {'─'*6}")
    for i, r in enumerate(all_results):
        marker = "►" if i == best_idx else " "
        print(f"  {marker}{r['sf']:>5} {r['mn']:>4}  {r['recall']:>7.1f}% {r['precision']:>7.1f}% {r['f1']:>7.3f} {r['specificity']:>7.1f}% {r['fp_per_img']:>8.3f} {r['avg_iou']:>5.2f}")
    print(f"  {'─'*63}")
    print(f"  ► = Meilleure config par F1-Score")

    # ── Rapport détaillé pour la meilleure configuration ──
    r = best
    print(f"\n  Rapport détaillé (scaleFactor={r['sf']}, minNeighbors={r['mn']}) :")
    print(f"  ┌────────────────────────────────┬──────────────┐")
    ...
    # ── Recommandation Raspberry Pi Zero V1 ──
    print(f"\n  Raspberry Pi Zero V1 (ARM11 1GHz, 512MB RAM, image 320×240) :")
    print(f"    Estimations de vitesse selon scaleFactor :")
    print(f"      SF=1.05 → ~0.5-1 FPS  (très précis mais trop lent pour du temps réel)")
    print(f"      SF=1.1  → ~1-2 FPS    (bon recall, acceptable pour navigation lente)")
    print(f"      SF=1.2  → ~2-4 FPS    (bon compromis vitesse/précision)")
    print(f"      SF=1.3  → ~4-6 FPS    (rapide, peut manquer des petits objets)")
    print(f"    minNeighbors : impact négligeable sur la vitesse.")
    print(f"    Note : LBP est ~3-5× plus rapide que HAAR à l'exécution.")
    print(f"    Astuce : utiliser minSize=(w,h) basé sur la distance minimale de")
    print(f"             détection pour limiter la pyramide et gagner en FPS.")

    # Trouver la meilleure config compatible Pi (SF >= 1.2)
    pi_results = [r for r in all_results if r['sf'] >= 1.2]
    if pi_results:
        best_pi = max(pi_results, key=lambda x: x['f1'])
        print(f"\n    Recommandation Pi Zero V1 :")
        print(f"      scaleFactor={best_pi['sf']}, minNeighbors={best_pi['mn']}")
        print(f"      (Recall={best_pi['recall']:.1f}%, Précision={best_pi['precision']:.1f}%, F1={best_pi['f1']:.3f})")

    print(f"\n  Meilleurs paramètres (qualité max, PC) :")
    print(f"    scaleFactor={best['sf']}, minNeighbors={best['mn']}")
    print(f"    (Recall={best['recall']:.1f}%, Précision={best['precision']:.1f}%, F1={best['f1']:.3f})")
...
def test_model(model, test_image_dir, positive=True, scaleFactor=1.1, minNeighbors=5):
    ...
    stats = {
        'tp': 0, 'fn': 0, 'fp': 0, 'tn': 0,
        'total_detections': 0,      # nombre total de bounding boxes retournées
        'multi_detect_count': 0,    # images avec >1 détection
        'iou_sum': 0.0,             # somme des meilleurs IoU (positives seulement)
        'iou_count': 0              # nombre d'IoU calculés
    }
    ...
    for img_file in tqdm(test_images, unit="img", colour="green", ncols=80):
        ...
        if positive:
            if n_det == 0:
                ...
                # IoU : meilleure détection vs image entière (mode plein cadre)
                h_img, w_img = img.shape[:2]
                best_iou = 0.0
                for (x, y, w, h) in detections:
                    # Intersection entre la détection et l'image entière (ground truth)
