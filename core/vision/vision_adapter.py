...
Vecteur d'état (33 + N classes):
  ...
  [24+N..30+N]: Zone features (7)         - détection multi-zones caméra
     24+N: front_line_detected            - 0 ou 1
     25+N: front_line_confirmed           - 0 ou 1
     26+N: front_offset_norm              - [-1, 1]
     27+N: corner_left_detected           - 0 ou 1
     28+N: corner_right_detected          - 0 ou 1
     29+N: corner_left_area_norm          - [0, 1]
     30+N: corner_right_area_norm         - [0, 1]
  [31+N]     : line_camera_offset (1)    - position de la ligne blanche [-1, 1]
  [32+N]     : line_camera_detected (1)  - 0 ou 1 (ligne visible par caméra)
  ...
  Note: les anciennes données (29-dim, sans zone features) sont zero-paddées
  vers 36-dim par dataset.py:pad_to_new_format() lors de l'entraînement.
...
class VisionAdapter:
    ...
    def state_dim(self) -> int:
        """Dimension du vecteur d'état (entrée) : 33 + N classes."""
    ...
    def get_state_vector(
        ...
        # --- Zone features (indices 24+N à 30+N) — détection multi-zones caméra ---
        ...
        state[zone_start + 3] = 1.0 if corner_left_detected else 0.0
        state[zone_start + 4] = 1.0 if corner_right_detected else 0.0
        state[zone_start + 5] = min(float(corner_left_area) / max_corner_area, 1.0)
        state[zone_start + 6] = min(float(corner_right_area) / max_corner_area, 1.0)

        # --- Détection de ligne par caméra (indices 31+N, 32+N) ---
    ...
    def debug_print_state(self, state: np.ndarray, label: Optional[np.ndarray] = None):
        ...
        # --- Zone features ---
        zone_start = imu_start + IMU_DIM
        print("  [Zones] front_det={:.0f}, front_conf={:.0f}, front_off={:.2f}".format(
            state[zone_start], state[zone_start + 1], state[zone_start + 2]))
        print("  [Zones] corner_L={:.0f} (area={:.3f}), corner_R={:.0f} (area={:.3f})".format(
            state[zone_start + 3], state[zone_start + 5],
